...
export default function ProfileByUsernamePage({ params }: { readonly params: Promise<{ username: string }> }) {
    ...
    const userServices = useMemo(() => {
        return profileUser ? services.filter((s) => s.sellerId === profileUser.id && s.isActive) : []
    }, [services, profileUser])

    const userReviews = useMemo(() => {
        return profileUser ? reviews.filter((r) => r.revieweeId === profileUser.id) : []
    }, [reviews, profileUser])
    ...
    }, [searchParams, isOwnProfile, currentUser?.role, router, username])
